...
[imm], [xmm], [ymm], and [zmm] if you're being size-wise. Also GPR, or SPR if you're being symbolic-wise.
...
it is, in case you're ignoring ABIs. If you don't, you're usually limited by conventions. These registers are all [imm] (covered later), and you can
manage their state by using the [stack].
...
Following to the size point of view, GPR are all [imm], which ranges in size from 16-bits to 64-bits. There is also the [SIMD] registers, which are
[xmm], [ymm], and [zmm]. In the same sequence, each having 128-bits, 256-bits, and 512-bits, they being from the features [AVX], [AVX2], and [AVX-512].
...
is to describe registers per se. If you want to read more about these, check: [Named Registers]. If you want to access all the registers, down here you can see a table covering all of the x86-64 registers.
...
| rdx | 64 | GPR | — | 3rd argument in SysV ABI; high half of result in mul/div paired with rax |
...
| rflags | 64 | SPR | — | Status and control flags (carry, zero, sign, overflow, direction, etc.) |
